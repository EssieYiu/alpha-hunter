...
以当前六个页面为范围：行情总览、自选、模拟持仓、Agent 助手、策略管理、策略回测；包含全局 Agent 抽屉、会话历史、参数化策略及后续新增的可信用户代码策略。先落地个人/小规模使用的研究工具、模拟交易与记账。真实券商下单、高频交易和面向不可信用户的任意代码执行不属于这次选型承诺。
...
一期允许用户新增参数化策略。后续增加了仅供单人可信使用的 Python 代码策略：通过 `StrategyBase.on_bar` 接入统一回测引擎，以子进程运行并限制时间与资源；具体格式见 [代码策略契约](code-strategies.md)。该子进程不是面对不可信用户的安全沙箱，Agent 工具仍只能创建参数化策略。
